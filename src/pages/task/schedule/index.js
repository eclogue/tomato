...
const Index = ({ dispatch, location, schedule }) => {
  ...
  const info = (
    <Descriptions title="Schedule" column={1} size="small" bordered={true}>
      ...
      <Descriptions.Item label="args"><Tag>{stringObject(task.args)}</Tag></Descriptions.Item>
      <Descriptions.Item label="Kwargs"><Tag>{stringObject(task.kwargs)}</Tag></Descriptions.Item>
      <Descriptions.Item label="Next run time"><Tag>{task.next_run_time || 'pause'}</Tag></Descriptions.Item>
      ...
      <Descriptions.Item label="Actions">
        <ButtonGroup>
          <Button disabled={!task.next_run_time} onClick={() => handleAction(task.id, 'pause')}>pause</Button>
          <Button disabled={task.next_run_time} onClick={() => handleAction(task.id, 'resume')}>resume</Button>
          <Button  onClick={() => handleAction(task.id, 'remove')}>remove</Button>
          <Button  onClick={() => handleAction(task.id, 'reschedule')}>reschedule</Button>
        </ButtonGroup>
      </Descriptions.Item>
    </Descriptions>
  )
