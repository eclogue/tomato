  ...
  return (
    ...
    <Col {...ColProps} xl={{ span: 4 }} md={{ span:4 }}>
      {getFieldDecorator('region', {
        initialValue: filter.region || undefined,
        rules: [
          {
            required: false,
          }
        ...
        <Select placeholder="data center" style={{ width: '100%' }}>
          {regions.map( item => <Option value={item._id} key={item._id}>{item.name}</Option>)}
        </Select>
      )}
      </Col>
